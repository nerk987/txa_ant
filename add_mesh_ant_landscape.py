...
class AntAddLandscape(bpy.types.Operator):
    bl_idname = "mesh.txa_landscape_add"
    ...
    def FindUniqueName(self, context):
        for i in range(1,99):
            if "ant"+str(i).zfill(2) not in context.scene.objects:
                return "ant"+str(i).zfill(2)
        return "Landscape"
    ...
    def execute(self, context):
        ...
        # print("Noise Basis add mesh start: ", self.basis_type)
        ...
        if self.ant_terrain_name != "":
            ...
            # new_name  = "Landscape"
            new_name = self.FindUniqueName(context)
        ...
        #
        # print("Noise basis add before store: ", self.basis_type)
        new_ob = store_properties(self, new_ob)
        # print("Noise basis after store from ob: ",  new_ob['basis_type'])
        # print("Noise basis after store from ob2: ",  new_ob.ant_landscape.basis_type)
        #Added due to change in the way the ENUMProperty works
        # new_ob['noise_basis'] = self.basis_type.items[0]
        # new_ob['vl_noise_basis'] = self.vl_basis_type.items[0]

        if self.auto_refresh is False:
            self.refresh = False
            
        # print("requesting regen")
        bpy.ops.mesh.txa_ant_landscape_regenerate('EXEC_DEFAULT')
